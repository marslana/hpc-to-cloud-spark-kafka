...
```
├── paper/
│   ├── main.tex                        # Paper source (LNCS format, includes TikZ figures)
...
│   │   └── approach*.sh                 # Per-approach automation
...
- **Software:** Apache Kafka 2.8 + ZooKeeper 3.7, Apache Spark 3.4, Python 3.10+
...
The paper has 4 figures. Two (native Kafka throughput, Spark vs. native comparison) are PDF files in `paper/figures/`. The other two (architecture diagram, pipeline Gantt timeline) are TikZ code drawn directly in `main.tex`. To regenerate the benchmark PDFs from raw data:
