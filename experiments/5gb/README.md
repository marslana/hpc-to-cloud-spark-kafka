...
```bash
# From your laptop
scp -P 8022 -r "experiments/5gb" mtariq@access-aion.uni.lu:~/experiments/5gb
```
...
cd /Users/arslan.tariq/mock\ skyplane/skyplane
...
Copy to laptop:
```bash
scp -P 8022 -r mtariq@access-aion.uni.lu:~/5gb_results/ \
    "/Users/arslan.tariq/Desktop/HPC data/experiments/5gb/results/"
```

## Expected Results (estimates based on 1GB measurements)

| # | Approach | Est. E2E | Est. First Record |
|---|----------|----------|-------------------|
| 1 | Cloud-Side Processing | ~370s | ~300s |
| 2 | Direct Producer | ~955s | ~7s |
| 3 | HPC-Side Processing | ~190s | ~130s |
| 4 | MM2 Replication | ~1545s | ~40s |
| 5 | SkyHOST Transfer | ~200s | ~15s |
...
- **Approach 2**: spark_read, spark_filter, kafka_produce (WAN), consumer_first_record, e2e
